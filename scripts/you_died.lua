...
--are you a villager?
isVillagerSkin = false

if (
	    (
		    isVillagerSkin and (boyfriendName == 'villager-player' or boyfriendName == 'villager')
	    )
	    or not isVillagerSkin
    )
    and not lowQuality then
	...
	music = {
		'deathScreen/aaron cherof crescent dunes',
		'deathScreen/c418 11',
		...
		'deathScreen/captainsparklez minecraft style',
		'deathScreen/kumi tanioka pokopoko',
		'deathScreen/lena raine an ordinary',
		'deathScreen/lena raine floating dream',
		'deathScreen/lena raine one more day',
		'deathScreen/lena raine otherside',
		'deathScreen/lena raine rubedo'
	}
	...
	function onCreatePost()
		...
		if isVillagerSkin then
			precacheSound('deathScreen/villager-death')
		end
	end
	...
	function onCustomSubstateCreatePost(n)
		if n == 'gamerrrr' then
			...
			doTweenZoom('camz', 'camGame', 1.4, 40, 'quintOut')
	...
	function onTimerCompleted(t)
		if t == 'gameOverMusic' then
			if not stop then
				playMusic(music[getRandomInt(1, table.getn(music), true)], 1, true, 'musicMinecraft')
			end
		end
